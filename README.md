...
指定ディレクトリ内のファイルが `history` フォルダに既に存在しないかをチェックする Windows 向け CLI ツール。RPA ワークフローにおけるファイルの二重処理防止などに活用できます。
...
│   ├── data_20240101.txt
│   └── data_20240102
├── data_20240103.txt      ← チェック対象（処理前ファイル）
├── data_20240104
└── dupcheck.exe
```
...
dupcheck.exe -target C:\data -history C:\data\history -log
...
[dupcheck] 対象ディレクトリ : C:\data
[dupcheck] historyディレクトリ: C:\data\history

[重複] data_20240103.txt
[正常] data_20240105.txt    ← -verbose 時のみ表示
